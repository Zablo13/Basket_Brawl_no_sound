...
func get_input():
	...
	if dashed == true or bump == true: #Keine Beeinflussung vom Dash
		return
	var dir = Input.get_axis("move_left1" , "move_right1") #Achse aus links rechts bestimmen
...
func jump():#Springen :)
	...
	if Input.is_action_just_pressed("jump1") and jumps > 0: #Abfrage Sprung Input
		jumps -= 1
		velocity.y = jump_speed #Die Sprungkraft
		if jumps == 0:
			floatable()
	#Jetpack zünden			
	if Input.is_action_pressed("jump1") and can_float == true:
		floating = true
	#Jetpack aus ohne Druck auf Jump			
	if Input.is_action_just_released("jump1"):
		floating = false
	#Fast fall mit Druck nach unten			
	if Input.is_action_pressed("move_down1") and floating == false and !Input.is_action_pressed("catch1"):
		fast_fall = true
	#Fast fall aus ohne Druck nach unten
	if Input.is_action_just_released("move_down1") or is_on_floor():
		fast_fall = false
...
func dashing():
	#Hier der Dash
	if dash == true:
		if Input.is_action_just_pressed("dash_right1"): #Input abfragen
			dash = false
			dashed = true
	...
	if Input.is_action_pressed("dash_left1"): #Input abfragen
		power = 0
...
func get_throw_dir() -> Vector2:
	...
	new_throw_dir = Input.get_vector("move_left1" , "move_right1" , "move_up1" , "move_down1") #Vektor aus Stickrichtung erzeugt
...
func throw():
	...
	if Input.is_action_just_pressed("catch1") and held == false and stun == false and microstun == false and punching == false: #Input lauschen
		$Center.look_at(throw_dir *100000) #Hand folgt Stickrichtung
		power = 0
		catcher() #Hand weiß färben und zum Fangen aktivieren
		
	if Input.is_action_pressed("catch1"): #lauscht auf Input
		$Center.look_at(throw_dir *100000) #Hand folgt Stickrichtung
	...
	if Input.is_action_just_released("catch1") and held == true: #Der normale Wurf
		off() #Ball loslassen
...
func punish():
	...
	if Input.is_action_just_pressed("punch1") and punching == true and whirl == false and p3 == false:
		punching = false
		can_punch = false
		can_toss = false
		count2 = 0.0
	...
	#Zuschlagen
	if Input.is_action_just_pressed("punch1") and held == false and stun == false and can_punch == true and parry == false and whirl == false and punching == false and p3 == false:#Input lauschen
		can_punch = false
		punching = true
		can_toss = false
		count = 0.0
		...
		if Input.is_action_pressed("move_left1") and check == true:
			whirling_left()
		if Input.is_action_pressed("move_right1") and check == true:
			whirling_right()
		if Input.is_action_pressed("move_up1") and check == true:
			whirling_up()
		if Input.is_action_pressed("move_down1") and check == true:
			whirling_down()			
...
func hitter():#Die Spielerhand
	...
	for body in %Catcher.get_overlapping_bodies(): #Kollisionsabfrage wer ist in der Hand
		...
		#Ball fangen
		if Input.is_action_pressed("catch1") and held == false and stun == false and can_hold == true:
...
func counter():
	...
	if Input.is_action_just_pressed("drop1") and parrycooler == false and held == false and stun == false and punching == false:
		parade()
...
func drop():#P!
	...
	if power > 200: #Power auf 200 begrenzen
		power = 200	
	if Input.is_action_just_pressed("drop1") and held == true: #Gegen sofortigen Konter bei Ballabgabe wird Parry blockiert
		if Input.is_action_pressed("move_left1") and held == true: #P! left / right
			off()
...
func toss():#P!
	#Die Schmetterwürfe
	if Input.is_action_pressed("punch1") and held == true and can_toss == true: #Schmetter hinten
		if Input.is_action_pressed("move_left1"): #P! rechts / links
...
func henshin():
	...
	if dashed == true:
		...
		if dir == Vector2.ZERO and Input.is_action_pressed("dash_right1"): #Ohne Richtung mit Ball nach oben
			velocity = Vector2.ZERO
			check = true


func taunting():
	if Input.is_action_just_pressed("dash_left1") and held == false and stun == false and tauntcooler == false:
		taunt()
			...
			if stun == false:
				...
				if stun == false:
					...
					if stun == false:
						...
						$run2.visible = true
						velocity.y -= 150
						if Input.is_action_pressed("catch1"):
							punching = false
							$run2.visible = false
							$Playersprite.visible = true
							whirl = false
							...
							speed_up = false
							return		
						if Input.is_action_pressed("drop1") and parrycooler == false:	
							parrycooler = true
							punching = false
							$run2.visible = false
							$Playersprite.visible = true
						...
						if stun == false:
							...
							#Follow Ups		
							if Input.is_action_pressed("jump1") and whirl == true:
								velocity.y += 100
								
							if Input.is_action_pressed("dash_left1") and whirl == true:
								velocity.y -= 1500
								$run2.visible = false
								$Playersprite.visible = true
								
							if Input.is_action_pressed("punch1") and whirl == true:
								punching = true
						...
						if Input.is_action_pressed("move_down1") and stun == false and not is_on_floor():
							fast_fall = true
							stomp = true	
						whirl = false
						$run2.visible = false
...
func fire():
	if Input.is_action_pressed("move_down1"):
		p1 = true
	if Input.is_action_pressed("move_right1") and p1 == true: #P! R/L
		p1 = false
		p2 = true
	if Input.is_action_just_released("move_down1") and Input.is_action_pressed("move_right1") and p2 == true and can_punch == true:#P! R/L
		p2 = false
		p3 = true
		
		
func firing():
	if Input.is_action_pressed("punch1") and p3 == true and can_punch == true and stun == false:
		p3 = false
		can_punch = false
		can_toss = false
		count2 = 0.0
